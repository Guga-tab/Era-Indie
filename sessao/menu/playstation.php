<?php
  session_start(); 
  include_once('../../classes/usuario.php');
 if(isset($_SESSION['id_usuario']))
 {
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Era Indie</title>
    <link rel="icon" href="../../img/logo/icone pagina.png">
    <link rel="stylesheet" href="../../css/playstation.css">
    <link href="bootstrap-3.3.6-dist/css/bootstrap.min.css" rel="stylesheet" media="screen">
    <link href="bootstrap-3.3.6-dist/js/bootstrap.min.css" rel="stylesheet" media="screen">
</head>
<body>
    <header class="nav">
        <input type="checkbox" id="nav-check">
        <div class="nav-header">
          <div class="nav-title">
		       <a href="../index.php"><img src="../../img/logo/logo1-semfundo vermelha-branca.png" alt=""></a>
           <p>A era dos jogos indies é aqui!</p>
          </div>
        </div>
        <div class="nav-btn">
          <label for="nav-check">
            <span></span>
            <span></span>
            <span></span>
          </label>
        </div>
        
        <div class="nav-links">
            <ul>
            <li><a href="xbox.php" id="nav-xbox">Xbox</a></li>
            <li><a href="nintendo.php" id="nav-nintendo">Nintendo</a></li>
            <li><a href="playstation.php" id="nav-sony">Playstation</a></li>
            <li><a href="promo.php" id="nav-promo">Promoções</a></li>
            <li class="login"><a id="login" href="../../php/sair.php">Sair</a></li>
            </ul>
        </div>
      </header>

      <div class="modulo">
        <img src="https://adrenaline.com.br/uploads/chamadas/playstation-indies-tutorial-0.jpg" alt="" id="imgPSindie"> 
        <h2 class="titulo"><a href="../playstation/top5ps.php"> Conheça os jogos indie da Playstation. </a></h2>
        <p class="conteudoindieps"> Jogos indie lançados para o Playstation.</p>
      </div>
        
        <div class="modulo">
            <img src="https://cdn.cloudflare.steamstatic.com/steam/apps/1244090/capsule_616x353.jpg?t=1657223058" alt="" id="imgSea">
            <h2 class="titulo"><a href="../playstation/pslançamentos.php">Jogos Indie em lançamento no Playstation 4 e 5.</a></h2>
           <p class="conteudops">Jogos indie em lançamento para o Playstation.</p>
         </div>

        <div class="modulo">
            <img src="https://static.gamevicio.com/imagens_up/big/80/sony-esta-tao-preocupada-com-call-of-duty-que-enviou-jim-ryan-para-se-reuniu-com-os-reguladores-da-uniao-europeia-079602.jpg" id="imgJim">
            <h2 class="titulo"><a href="../playstation/ceops.php"> Jim Ryan, o atual CEO da Sony Interactive <br>Entertainment, retoma o interesse em jogos indie.</a></h2>
            <p class="conteudojim">Chefão da sony apoia os jogos indie.</p>
        </div>

    <div class="moduloaside">
        <center>
            <h2 class="asidetitulo">Principais Títulos</h2>
            <a href="https://www.playstation.com/pt-br/games/kena-bridge-of-spirits/" target="_blank"> <img src="https://cdn1.epicgames.com/7c53d7dc86494d5abcdc974b0ef0a843/offer/EGS_KenaBridgeofSpirits_EmberLab_S6-1200x1600-e701bd947727ee14b81626c7f9757e4f.jpg" alt="kena" id="kena"></a>
            <a href="https://store.playstation.com/pt-br/product/UP4395-PPSA02593_00-THEASCENT00000US" target="_blank"><img src="https://www.mobygames.com/images/covers/l/754730-the-ascent-windows-apps-front-cover.jpg" alt="ascent" id="ascent"></a>
            </center>
    </div>

      <center>
        <footer>
         <div class="rodape">
            <Strong><p>Informações:</p></Strong>
            <p>Email: eraindie5@gmail.com</p>
            <p>Apoio: Etec Centro Paula Souza</p>
          </div>
          <div class="rodape2">
          <Strong><p>Nos siga também nas redes sociais:</p></Strong>
          <p>facebook: @eraindie</p></a>
          <p>Instagram: @eraindie</p></a>
          </div>
          <p class="copy">© Copyright EraIndie 2022 - 2022. todos os direitos reservados.</p>
        </footer>
        </center>
      </body>
      </html>
<?php
}
else
{
  echo "
  <script>
    window.alert('Não permitido')
    window.location.href='../../php/login.php';
  </script>";
  
}
?>